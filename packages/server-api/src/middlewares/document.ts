...
import { EsKoaMiddleware, EsKoaContext } from '../es-koa';
...
const documentMiddlewares: DocumentMiddlewares = {
    upload: async (ctx) => {
        ...
        await control.documentRevision.uploadChunk(
            {
                db,
    ...
    downloadToken: async (ctx) => {
        ...
        const documentRevisionId = await dao.documentRevision.idByDocumentIdAndRev(
            db,
            documentId,
            parseInt(revision)
        );
    ...
    download: async (ctx) => {
        ...
        const auth = {
            userId,
            userPerms: config.rolePolicies.user.permissions(roles),
        };
