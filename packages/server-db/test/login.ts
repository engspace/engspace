...
async function createLogins(db: Db, users: Promise<Dict<User>>): Promise<void> {
    ...
    for (const name in usrs) {
        await dao.login.create(db, usrs[name].id, `${name}.pass`);
    }
}
...
describe('LoginDao', () => {
    ...
    describe('Create login', async () => {
        after('Delete logins', () => pool.connect(db => dao.login.deleteAll(db)));
        it('should create a login with password', async () => {
            await pool.connect(db => dao.login.create(db, users.a.id, 'correct_password'));
            const ok = await pool.connect(db =>
                dao.login.checkById(db, users.a.id, 'correct_password')
            );
            const nok = await pool.connect(db =>
                dao.login.checkById(db, users.a.id, 'incorrect_password')
            );
    ...
    describe('Login procedure', async () => {
        ...
        it('should return user info if log with username', async () => {
            const res = await pool.connect(db => dao.login.login(db, 'b', 'b.pass'));
        ...
        it('should return user info if log with email', async () => {
            const res = await pool.connect(db => dao.login.login(db, 'a@engspace.net', 'a.pass'));
        ...
        it('should return null if incorrect password', async () => {
            const res = await pool.connect(db => dao.login.login(db, 'a', 'b.pass'));
            expect(res).to.be.null;
        });

        it('should return null if incorrect username', async () => {
            const res = await pool.connect(db => dao.login.login(db, 'nobody', 'a.pass'));
            expect(res).to.be.null;
        });
    });
    ...
    describe('Check login', async () => {
        ...
        it('should accept correct password', async () => {
            const ok = await pool.connect(db => dao.login.checkById(db, users.a.id, 'a.pass'));
            expect(ok).to.be.true;
        });
        it('should reject incorrect password', async () => {
            const nok = await pool.connect(db => dao.login.checkById(db, users.a.id, 'b.pass'));
            expect(nok).to.be.false;
        });
    });
    ...
    describe('Patch login', async () => {
        ...
        afterEach('Delete logins', () => pool.connect(db => dao.login.deleteAll(db)));

        it('should patch an existing password', async () => {
            await pool.connect(db => dao.login.patch(db, users.a.id, 'new_password'));
            const ok = await pool.connect(db =>
                dao.login.checkById(db, users.a.id, 'new_password')
            );
            const nok = await pool.connect(db =>
                dao.login.checkById(db, users.a.id, 'not_new_password')
            );
            expect(ok).to.be.true;
            expect(nok).to.be.false;
        });
    ...
    describe('Delete', async () => {
        ...
        it('should delete a login', async () => {
            ...
            const ok = await pool.connect(db => dao.login.checkById(db, users.a.id, 'a.pass'));
            await pool.connect(db => dao.login.deleteById(db, users.a.id));
            const nok = await pool.connect(db => dao.login.checkById(db, users.a.id, 'a.pass'));
